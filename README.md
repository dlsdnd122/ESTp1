...
### 📸 개발 화면
<img width="1466" height="825" alt="스크린샷 2025-08-01 오후 5 02 26" src="https://github.com/user-attachments/assets/b781314c-fc7a-4ac6-a505-e9868c3412a8" />




### ✅ 진행된 작업 목록
- [x] 리드미 작성 - 완성은 아니고 아직 덜 적음 ppt 만들면서 코드도 채워 넣을 예정
- [x] 피그마로 와이어프레임 만들기 - 처음시작할때부터 만들었어야 했는데 생각못함
- [x] 계획표 작성, 수정
- [x] 영상 비율 16:9로 고정
- [x] 썸네일 영상 미리보기 도전과제 구현

---

## ❗ 어려웠던 점 및 느낀 점

- 썸네일 영상 미리보기를 어떻게 구현할지 어제부터 생각을 해봤다 -> img 태그로 지금 썸내일 이미지를 구현함
- 이걸 js를 이용하여 호버를 할 경우 iframe 태그로 바꾸고 Id, autoplay같은 속성값을 넣어주면 어떨까 라는 생각을 하게됨
- gpt에게 질문해보니 가능하다고 답변 받고 구현을 하였음
- 지금 치명적인 오류를 하나 찾았는데, video.html파일로 넘어가서 사이드바를 열면 구독, shorts 버튼이 있는데
- 이 버튼들을 누르면 구독, shorts 페이지로 넘어갈 수가 없음
- 이유는 구독, shorts페이지의 경우 지금 main태그만을 fetch 함수를 통해서 index.html로 받아오는데,
- 그렇기에 video.html의 main 태그만이 바뀌게 되어 원하는 화면을 구현할 수 없음
- 처음 index.html에서 shorts, 구독을 구현할때 이번에 비디오id를 받아온 것 처럼 그렇게 구성했으면
- 어렵지 않게 구현했을텐데, 괜히 main태그만을 가져올려고 했다가 복잡하게 되버렸음
- 아.. 다음부터는 이런일 없이 신경써서 진행해야겠음 시간이 많다면 다 뜯어 고쳤을텐데 시간이 없기에 못할꺼같음
...
### ✅ 진행된 작업 목록
- [x] 노션에 발표자료, 배포주소 등 최종 마무리 작업물 제출
- [x] 팀원들과 서로 발표
- [x] 대표 발표자 선정
- [x] 내가 대표 발표자로 선정됨
